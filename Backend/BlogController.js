...
const blogInsert = async (req, res) => {
  const { blogID, bloggerName, bloggerPosition, title, content } = req.body;
  // Find the latest blog with the highest blogID
  const latestBlog = await blogModel.findOne().sort({ blogID: -1 });
  let nextBlogID = 1;
  // Default value if no blogs exist
  if (latestBlog) {
    // If there's a latest blog, increment its blogID by 1
    nextBlogID = latestBlog.blogID + 1;
  }
